/*
 * partition_scotch.hpp -
 * @author: Jonathan Beard, Qinzhe
 * @version: Tue Mar 07 12:59:12 2023
 *
 ...
 */
#ifndef RAFT_PARTITION_PARTITION_SCOTCH_HPP
#define RAFT_PARTITION_PARTITION_SCOTCH_HPP  1
#ifdef USE_PARTITION
#include <cmath>
#include <cstdio>
#include <cstdlib>
...
class PartitionScotch : public Partition
{

using container_type = typename kernelset_t;

public:
    PartitionScotch() : Partition() {}
...
private:
                       ...
                       const core_id_t cores )
    {
        if( c.size() <= cores )
        {
            ...
            for( auto *kernel : c )
            {
                (this)->set_group_for_kernel( *kernel, i );
                i++;
            }
            return( true );
        }
                     ...
                     void *weight );
    {
         ...
         */
         ...
         {
             (this)->set_group_for_kernel( *( *it ), table->partition[ i ] );
         }
        /** call exit graph **/
        delete( table );
...
#endif /** end USE_PARTITION **/

}; /** end PartitionScotch decl **/

} /** end namespace raft **/

#endif /* END RAFT_PARTITION_PARTITION_SCOTCH_HPP */
