/**
 * fifo.hpp - the interfaces of FIFO
 ...
 */
#ifndef RAFT_ALLOCATE_FIFO_HPP
#define RAFT_ALLOCATE_FIFO_HPP  1
#include <cstddef>
#include <typeinfo>
#include <iterator>
#include <list>
...
public:
    ...
    template< class T >
    using pop_range_t = std::vector< std::pair< T , signal::value_t > >;
    ...
    public:
       ...
       autorelease( FIFO &fifo ) :
           fifo( fifo ), item( fifo.peek< T >( &signal ) ) {}
       /** copy constructor **/
       ...
       /**
        * *operator - dereference this auto-release object to
        ...
        */
       const signal::value_t& sig() const noexcept
       {
          return( (this)->signal );
       }

       /**
        * copiedManually - mark this object has been copied to
        * other place manually (not via the copy constructor).
        * This is meant to assist FIFOFunctorT to create
        * autorelease< DataRef, autotype > from
        * autorelease< T, autotype >.
        */
       void copiedManually()
       {
          copied = true;
    ...
    private:
       T                   *item;
       signal::value_t      signal = signal::none;
       FIFO                 &fifo;
       bool                 copied = false;
    };
    ...
    template < class T,
               typename std::enable_if<
                   Buffer::inline_nonclass_alloc< T >::value >::type* =
                   nullptr >
    T& allocate()
    {
        void *ptr( nullptr );
          ...
          std::vector< std::reference_wrapper< T > >
    {
        ...
        /** compiler should optimize this copy,
         * if not then it'll be a copy of referneces not full objects **/
        return( std::forward<
           std::vector<
     ...
     * @param   signal - const signal::value_t, default: NONE
     */
    virtual void send( const signal::value_t = signal::none ) = 0;
     ...
     * @param   signal - const signal::value_t, default: NONE
     */
    virtual void send_range( const signal::value_t = signal::none ) = 0;
     ...
     */
    template < class T >
    void push( const T &item, const signal::value_t signal = signal::none )
    {
     ...
     * @param   item - T&
     * @param   signal - signal::value_t
     */
    template< class T >
    void pop( T &item, signal::value_t *signal = nullptr )
    {
     ...
     * @param   items    - std::vector< std::pair< T, signal::value_t > >&
     ...
     * being used.
     * @param   signal - signal::value_t, default: nullptr
     * @return T&
     */
    template< class T,
              typename std::enable_if<
                  Buffer::inline_alloc< T >::value >::type* = nullptr >
    T& peek( signal::value_t *signal = nullptr )
    {
        void *ptr( nullptr );
    ...
    template< class T,
              typename std::enable_if< Buffer::ext_alloc<
    ...
    auto peek_range( const std::size_t n ) -> autorelease< T, peekrange >
    {
        /** FIXME: still not implemented yet
         * for externally allocated objects **/
        assert( false );
        void *ptr = nullptr;
        void *sig = nullptr;
        std::size_t curr_pointer_loc( 0 );
...
protected:
     ...
     * @return signal::value_t
     */
    virtual signal::value_t signal_peek() = 0;
    /**
     ...
     * @param sig - signal::value_t, signal to be sent
     */
    virtual void inline_signal_send( const signal::value_t sig ) = 0;
     ...
     * @param   ptr - void*
     * @param   signal - signal::value_t reference
     */
    virtual void local_push( void *ptr, const signal::value_t &signal ) = 0;
     ...
     * @param   ptr_end   - void*
     * @param   signal    - const signal::value_t&
     */
     ...
     * @param   signal - signal::value_t*
     */
    virtual void local_pop( void *ptr, signal::value_t *signal ) = 0;
