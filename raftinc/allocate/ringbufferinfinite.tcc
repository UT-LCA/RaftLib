/**
 * ringbufferinfinite.tcc - specializations of RingBufferBase with
 * Infinite as the buffer data type. The content of this
...
template < class T >
class RingBufferBase< T,
                      Buffer::Type::Infinite,
                      Buffer::inline_nonclass_alloc_t< T > > :
                          public FIFOAbstract< T, Buffer::Type::Infinite >
{
public:
    /**
     ...
     * data structures.
     */
    RingBufferBase() : FIFOAbstract< T, Buffer::Type::Infinite >()
    {
    }
    ...
    virtual signal::value_t get_signal()
    {
#if 0
    ...
    virtual bool send_signal( const signal::value_t &signal )
    {
        UNUSED( signal );
        return( true );
    }
...
protected:
    ...
    virtual std::size_t local_allocate_n( void *ptr, const std::size_t n )
    {
        ...
        auto *container( reinterpret_cast<
                std::vector< std::reference_wrapper< T > >* >( ptr ) );
    ...
    virtual void local_push( void *ptr, const signal::value_t &signal )
    {
        ...
        /** a bit awkward since it gives the same behavior as the actual
         * queue **/
        (this)->datamanager.get()->signal[ 0 ].sig = signal;
                               ...
                               const std::size_t iterator_type )
    {
        ...
        const std::map< std::size_t,
                  std::function< void (void*,void*,const signal::value_t&) > > func_map
                    = {{ typeid( it_list ).hash_code(),
                         [ & ]( void *b_ptr, void *e_ptr, const signal::value_t &sig )
                         {
...
#endif /* END RAFT_ALLOCATE_RINGBUFFERINFINITE_TCC */
